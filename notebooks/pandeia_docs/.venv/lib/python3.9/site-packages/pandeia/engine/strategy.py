...
class Strategy(DefaultConfig):
    ...
    **kwargs: list of keyword/value pairs
    ...
    def _create_covariance_matrix(self, my_detector_signal, my_detector_noise, subscripts=None):
        ...
        if my_detector_signal.the_detector.rn_correlation:
            c_ij = np.zeros((nn, nn))
    ...
    def _error_sum(self, a_ij, product_subscripts, my_detector_signal, my_detector_noise):
        ...
        for product_subscript in product_subscripts:
            ...
            a_ij_bg_raveled = np.asarray(a_ij[product_subscript], dtype=float)
    ...
    def _fill_array(self, n, value):
        ...
        array = np.empty(n, dtype=np.dtype('i4'))
        array.fill(value)
        return array
...
class Coronagraphy(ImagingApPhot):
    ...
    def _create_weight_matrix(self, my_detector_signal_list, my_detector_noise_list):
        ...
        if len(my_detector_signal_list) != 3:
            message = 'Coronagraphic observations require three dithers (one on target, one on a PSF subtraction star and one of the unocculted target star)'
            raise UnsupportedError(value=message)
    ...
    def _make_dither_weights(self, exposure_list):
        ...
        if self.do_contrast:
            ...
            if self.psf_subtraction == "optimal":
                ...
                sizea = int(np.round(sizea/2.0))
                sizeb = int(np.round(sizeb/2.0))
    ...
    **kwargs: list of keyword/value pairs
    ...
    def _sanity_checks(self):
        ...
        for d in self.dithers:
            if len(d['on_source']) != len(self.instrument.get_slit_pars()['multishutter']):
    ...
    def _create_weight_matrix(self, my_detector_signal_list, my_detector_noise_list):
        ...
        height = self.instrument.get_slit_pars()['xdisp']
        shutter_locations = self.instrument.get_slit_pars()['multishutter']
...
class MSAShutterApPhot(Strategy):
    ...
    **kwargs: list of keyword/value pairs
    ...
    def _create_weight_matrix(self, my_detector_signal_list, my_detector_noise_list):
        ...
        height = self.instrument.get_slit_pars()['xdisp']
        width = self.instrument.get_slit_pars()['disp']
        shutter_locations = self.instrument.get_slit_pars()['multishutter']
...
class SOSS(Strategy):
    ...
    **kwargs: list of keyword/value pairs
    ...
    def _create_weight_matrix(self, my_detector_signal_list, my_detector_noise_list):
        ...
        y_excess = signal.spatial_grid_unbinned.ny / 2.0
        ...
        # get the detector pixel offset from the first wavelength
        wave_off_pix = signal.detector_pixel_list[self.order-1][0]
