...
import pandeia.engine.helpers.background.jwst.accessor_globals as glb

from pandeia.engine.helpers.background.jwst.mod_healpix_func import ang2pix_ring

SL_NWAVE = len(glb.wavelist)
THERMAL_FNAME = os.environ['THERMAL_FNAME']
...
def get_sl(ra, dec, mjd2000, ra_dec_str, date_str):
    ...
    if iday_pt == -1:
        msg = 'Specified position ({}) is not observable on {}.\nUse of the ' \
              '<a href="https://jwst-docs.stsci.edu/display/JPP/JWST+General+Target+Visibility+Tool+Help" '\
              'target="_blank">General Target Visibility Tool (GTVT)</a> is recommended for determining the visibility'\
              ' of the specified position on the specified date'.format(ra_dec_str, date_str)
